...
def generate_markdown_report(lolo_results, bootstrap_results, entropy_results, fairness_results, output_file):
    ...
    # 标题
    lines.append("# Advanced Statistical Analysis of VLM-Dixit Games")
    lines.append("")
    lines.append("This document presents advanced statistical analyses based on the 21 completed matches (504 rounds total).")
    lines.append("")
    lines.append("---")
    lines.append("")
    
    # 1. LOLO分析
    lines.append("## 1. Leave-One-Listener-Out (LOLO) Analysis")
    lines.append("")
    lines.append("### Methodology")
    lines.append("For each round, we systematically remove one listener's vote and recalculate the storyteller's score. This tests the **robustness** of storytelling success to individual listener variations.")
    lines.append("")
    lines.append("### Results by Model")
    lines.append("")
    lines.append("| Model | Original Score | Avg LOLO Score | Avg Δ | Std Δ | Stability Index |")
    lines.append("|-------|----------------|----------------|-------|-------|-----------------|")
